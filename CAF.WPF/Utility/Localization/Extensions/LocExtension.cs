...
using CompositeApplicationFramework.Utility.Localization.TypeConverters;
...
namespace CompositeApplicationFramework.Utility.Localization.Extensions
{
    ...
    public class LocExtension : NestedMarkupExtension, INotifyPropertyChanged, IDictionaryEventListener, IDisposable
    {
        ...
        /// <summary>
        ///     Gets the extension that is bound to a given target. Please note, that only the last endpoint of each extension can
        ///     be evaluated.
        /// </summary>
        ...
        public static LocExtension GetBoundExtension(object target, string property, int propertyIndex = -1)
        {
            foreach (var ext in LocalizeDictionary.DictionaryEvent.EnumerateListeners<LocExtension>())
            {
                ...
                if (ep.TargetObjectReference.Target == target && epProp == property
        ...
        private static string GetPropertyName(object property)
        {
            ...
            var info = property as PropertyInfo;
            if (info != null)
            {
                epProp = info.Name;
            }
        ...
        public LocExtension()
        {
            ...
            OnFirstTarget = () => { LocalizeDictionary.DictionaryEvent.AddListener(this); };
        }
        ...
        public void ResourceChanged(DependencyObject sender, DictionaryEventArgs e)
        {
            ...
            var targetDOs = (from p in GetTargetPropertyPaths() select p.EndPoint.TargetObject as DependencyObject);
            ...
            foreach (var dObj in targetDOs)
            {
                ...
                while (doParent != null)
                {
                    ...
                    {
#if !SILVERLIGHT
                        var parent = doParent as FrameworkContentElement;
                        var doParent2 = parent != null ? parent.Parent : doParent.GetParent(true);
        ...
        private static void ClearItemFromResourceBuffer(DictionaryEventArgs dictionaryEventArgs)
        {
            if (dictionaryEventArgs.Type != DictionaryEventType.ValueChanged ||
                !(dictionaryEventArgs.Tag is ValueChangedEventArgs)) return;
            ...
            foreach (var key in _resourceBuffer.Keys.ToList()
                .Where(key => key.EndsWith(args.Key))
                .Where(key => ci == null || key.StartsWith(ci.Name)))
            {
                lock (ResolveLock)
                {
        ...
        public override object FormatOutput(TargetInfo endPoint, TargetInfo info)
        {
            ...
            var o = endPoint.TargetObject as FrameworkElement;
            if (o != null)
            {
                epName = o.GetValueSync<string>(FrameworkElement.NameProperty);
            }

            else if (endPoint.TargetObject is FrameworkContentElement)
            {
                epName =
                    ((FrameworkContentElement) endPoint.TargetObject).GetValueSync<string>(
            ...
            else
            {
                ...
                lock (ResolveLock)
                {
                    ...
                    else
                    {
                        ...
                        if (input == null)
                        {
                            var resKeyName = LocalizeDictionary.Instance.GetFullyQualifiedResourceKey(epName,
                                targetObject);
        ...
        public static TValue GetLocalizedValue<TValue>(
            string key,
        ...
        ///     True if the resolve was success, otherwise false.
        /// </returns>
        public bool ResolveLocalizedValue<TValue>(
            out TValue resolvedValue,
            CultureInfo targetCulture,
            DependencyObject target)
        {
            ...
            lock (ResolveLock)
            {
                ...
                else
                {
                    ...
                    if (!(result is TValue)) return resolvedValue != null;
        ...
        public bool SetBinding(object targetObject, object targetProperty, int targetPropertyIndex)
        {
            var existingBinding = (from info in GetTargetPropertyPaths()
                where
                    (info.EndPoint.TargetObject == targetObject)
                    && (info.EndPoint.TargetProperty == targetProperty)
            ...
            var result =
                ProvideValue(
                    new SimpleProvideValueServiceProvider(
                        targetObject,
                        targetProperty,
                        targetPropertyType,
                        targetPropertyIndex));
            ...
            SetPropertyValue(
                result,
                new TargetInfo(targetObject, targetProperty, targetPropertyType, targetPropertyIndex),
                false);
