...
namespace CompositeApplicationFramework.Utility.Localization.Providers
{
    ...
    public class CsvLocalizationProvider : CSVLocalizationProviderBase
    {
        ...
        {
            return target?.GetValueOrRegisterParentNotifier<string>(
        ...
        {
            return
                target?.GetValueOrRegisterParentNotifier<string>(
                    CsvEmbeddedLocalizationProvider.DefaultAssemblyProperty,
                    ParentChangedAction,
                    _parentNotifiers);
        }
        ...
        public override object GetLocalizedObject(string key, DependencyObject target, CultureInfo culture)
        {
            ...
            while (!Equals(culture, CultureInfo.InvariantCulture))
            {
                csvPath = Path.Combine(
                    csvDirectory,
                    dictionary + (string.IsNullOrEmpty(culture.Name) ? "" : "." + culture.Name) + ".csv");
            ...
            if (!File.Exists(csvPath))
            {
                ...
                if (!File.Exists(csvPath))
                {
                    OnProviderError(
                        target,
                        key,
                        "A file for the provided culture " + culture.EnglishName + " does not exist at " + csvPath + ".");
                    return null;
                }
            }
