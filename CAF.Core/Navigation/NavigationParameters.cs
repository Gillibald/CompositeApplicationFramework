...
namespace CompositeApplicationFramework.Navigation
{
    ...
    public class NavigationParameters : INavigationParameters
    {
        ...
        private static T GetValue<T>(string key, IEnumerable<KeyValuePair<string, object>> parameters)
        {
            foreach (var kvp in parameters)
            {
                if (string.Compare(kvp.Key, key, StringComparison.Ordinal) != 0) continue;
                ...
                if (typeof(T).GetTypeInfo().IsAssignableFrom(kvp.Value.GetType().GetTypeInfo()))
                {
                    return (T)kvp.Value;
                }
        ...
        public override string ToString()
        {
            ...
            if (_entries.Count <= 0) return queryBuilder.ToString();
