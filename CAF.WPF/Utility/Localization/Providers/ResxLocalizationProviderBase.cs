...
namespace CompositeApplicationFramework.Utility.Localization.Providers
{
    ...
    public abstract class ResxLocalizationProviderBase : DependencyObject, ILocalizationProvider
    {
        ...
        private static bool IsFileOfInterest(string f, string dir)
        {
            ...
            return f.EndsWith(".resx", StringComparison.OrdinalIgnoreCase)
        ...
        protected ResourceManager GetResourceManager(string resourceAssembly, string resourceDictionary)
        {
            ...
            if (AppDomain.CurrentDomain.FriendlyName.Contains("XDesProc")
                && ((now - _lastUpdateCheck).TotalSeconds >= 1.0))
            {
                ...
                {
                    foreach (
                        var process in Process.GetProcesses().Where(process => process.ProcessName.Contains(".vshost")))
                    {
                        ...
                        if (!files.Where(f =>
                        {
                            ...
                            return fileName != null && fileName.StartsWith(resourceAssembly);
                        }).Any()) continue;
            ...
            if (!TryGetValue(resManKey, out resManager))
            {
                ...
                foreach (var availableResource in availableResources)
                {
                    if (availableResource.EndsWith(resManagerNameToSearch)
                        && possiblePrefixes.Any(p => availableResource.StartsWith(p + ".")))
                    {
                ...
                try
                {
                    ...
                    foreach (
                        var c in
                            from c in cultures
                            let rs = resManager.GetResourceSet(c, true, false)
                            where rs != null
                            select c)
                    {
                        AddCulture(c);
