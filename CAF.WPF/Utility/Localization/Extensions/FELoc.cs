...
namespace CompositeApplicationFramework.Utility.Localization.Extensions
{
    ...
    public class FeLoc : FrameworkElement, IDictionaryEventListener, INotifyPropertyChanged, IDisposable
    {
        ...
        /// </summary>
        public static readonly DependencyProperty ForceCultureProperty = DependencyProperty.Register(
            "ForceCulture",
            typeof (string),
            typeof (FeLoc),
            new PropertyMetadata(null, DependencyPropertyChanged));
        ...
        private List<DependencyProperty> GetDependencyProperties(object element)
        {
            ...
            return
                (from mp in markupObject.Properties where mp.DependencyProperty != null select mp.DependencyProperty)
                    .ToList();
        }

        private void RegisterParentNotifier()
        {
            _parentChangedNotifier = new ParentChangedNotifier(
                this,
                () =>
                {
                    ...
                    foreach (var p in properties.Where(p => Equals(targetObject.GetValue(p), this)))
                    {
