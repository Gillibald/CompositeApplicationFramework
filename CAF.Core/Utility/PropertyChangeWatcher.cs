...
namespace CompositeApplicationFramework.Utility
{
    ...
    public sealed class PropertyChangeWatcher
    {
        ...
        private PropertyChangeWatcher([NotNull] INotifyPropertyChanged owner)
        {
            _owner = owner;
        ...
        public PropertyChangeWatcher AddWatcher(IList<string> propertyNames, Action handler)
        {
            ...
            Util.ThrowUnless(propertyNames.All(name => OwnerType.HasPublicInstanceProperty(name)),
                "The target object does not contain one or more of the properties provided");

            lock (_handlers)
            {
                foreach (var key in propertyNames)
                {
                    Util.ThrowUnless<ArgumentException>(!IsWatching(key),
                        "Must not already be watching property '{0}'".DoFormat(key));
                }
        ...
        public static PropertyChangeWatcher AddWatcher(INotifyPropertyChanged source, IList<string> propertyNames,
            Action handler)
        {
